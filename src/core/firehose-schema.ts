  ...
  tables: [
    {
      ...
      description:
        "One row per Bluesky Jetstream event. ts is UTC seconds; kind is commit/identity/account; collection is the record type (app.bsky.feed.post, app.bsky.feed.like, app.bsky.graph.follow, ...); operation is create/update/delete; lang is the first declared post language or empty; text_len is post text length in characters (0 for non-posts).",
    },
    {
      name: "sq_ticks",
