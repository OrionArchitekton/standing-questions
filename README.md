...
Ask the Bluesky firehose a question once. Get a living chart instead of a paragraph. Pin it, and an agent keeps re-evaluating on a schedule; when the picture materially changes, the agent reopens the question in the public reopened-threads feed with a visual before/after delta, not a wall of text.
...
1. **Live demo:** https://standing-questions.vercel.app (keyless; analytics SQL is gated read-only: single SELECT, allowlisted; the only user-facing write is Pin, rate-capped)
...
user turn --> Trigger.dev chat.agent() run (sq-chat, durable session per thread)
          ...
          Living Answer card: in-house SVG chart + one-line verdict + evidence drawer
                ...
                v
          standing_questions + told_ledger (Neon Postgres, OLTP; chat_id links the pin to its originating chat)
                |
   Trigger.dev scheduled tasks (prod, verified live):
     ...
     reeval-standing  */10  gate re-check -> evaluatePlan -> diffSnapshots vs told-baseline
                ...
                v
          Told-Ledger row + visual before/after Delta Card in the public reopened-threads feed
...
- Every scheduled standing-question reevaluation writes a self-telemetry tick to `sq_ticks` in ClickHouse (outcome silent/delta/error, eval latency).
...
- **Neon Postgres**: OLTP plane; standing questions and the Told-Ledger. The `sq_cdc` publication feeds a running ClickPipe (`sq-neon-cdc`) that CDCs `standing_questions` and `told_ledger` into ClickHouse; the OLTP-plus-OLAP join is live on the public standing-watch section (live ClickPipes CDC integration, targeting the optional Postgres CDC bonus category).
...
- The public demo is keyless; all firehose SQL is gated read-only (single SELECT, allowlisted). Demo writes are limited to rate-capped Pins (5/min/IP, 50-question cap) plus the durable chat runs each turn creates; per-IP rate limits are per serverless instance, not global.
